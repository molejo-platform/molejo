...
Install the Operator and Agent with `molejoctl platform runtime install`, then run
`molejoctl platform control-plane install`. A new installation creates separate ECDSA
P-256 trust roots:
...
trailing roots are removed. An idempotent `molejoctl platform control-plane install` run
