...
an amd64-only cluster, a programmed `fruto-system/fruto` Gateway, and the source
registry Secret `fruto-system/registry-pull`. It defaults to `--context
fruto-lab` and refuses another context unless both `FRUTO_KUBE_CONTEXT` and
