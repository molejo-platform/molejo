...
`molejoctl capability tls prepare` é uma receita opcional de conveniência para o day zero. A primeira receita instala o cert-manager e solicita um certificado Let's Encrypt por Cloudflare DNS-01. O cert-manager é responsável pela renovação depois que o comando termina.
