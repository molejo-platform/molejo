...
el mismo nombre. Una lista vacía mantiene privado al workload. `spec.exposure`,
`spec.slug` y `spec.port` se conservan solamente para migración legada.
...
cluster exclusivamente amd64, el Gateway `molejo-system/molejo` programado y el
Secret de origen del registry `molejo-system/registry-pull`. El target usa
`--context molejo-lab` por default y rechaza otro contexto, salvo cuando
`MOLEJO_KUBE_CONTEXT` y `MOLEJO_ALLOW_CUSTOM_CONTEXT=true` sustituyen explícitamente
esa protección.
