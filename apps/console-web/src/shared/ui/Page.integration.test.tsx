...
describe("TabNav", () => {
  it("marks only the tab for the current page as active", async () => {
    ...
    const sourceRoute = createRoute({
      ...
      component: () => <TabNav label="Configuração do App" items={[
  ...
  it("keeps a parent area active on one of its nested pages", async () => {
    ...
    const buildRoute = createRoute({
      ...
      component: () => <TabNav label="Áreas" items={[
        { label: "Visão geral", to: "/apps/$appId", params: { appId: "app-1" } },
        { label: "Entrega", to: "/apps/$appId/deployments", params: { appId: "app-1" }, activeTo: ["/apps/$appId/deployments", "/apps/$appId/builds", "/apps/$appId/releases"] },
      ]}/>,
    });
    const router = createRouter({ routeTree: rootRoute.addChildren([buildRoute]), history: createMemoryHistory({ initialEntries: ["/apps/app-1/builds/build-1"] }) });
