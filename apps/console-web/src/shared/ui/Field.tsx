...
export function TextareaField({ label, helper, error, id: providedId, required, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement> & FieldMeta) {
  ...
  const id = providedId ?? generatedId;
  const descriptionId = helper || error ? `${id}-description` : undefined;
  return <div className={`${error ? "field invalid" : "field"}${required ? " is-required" : ""}`}><label htmlFor={id}><span>{label}</span></label><textarea id={id} required={required} aria-invalid={Boolean(error) || undefined} aria-describedby={descriptionId} {...props}/>{descriptionId && <small id={descriptionId} className={error ? "field-error" : "field-helper"}>{error ?? helper}</small>}</div>;
}
