...
export function ProjectOverviewPage() {
  ...
  return <ProjectLayout workspaceId={workspaceId} projectId={projectId}>{(name) => <><div className="summary-grid"><SummaryCard label="Apps" value={apps.data?.items.length ?? 0} to="/workspaces/$workspaceId/projects/$projectId/apps" params={{ workspaceId, projectId }}/><SummaryCard label="Environments" value={environments.data?.items.length ?? 0} to="/workspaces/$workspaceId/projects/$projectId/environments" params={{ workspaceId, projectId }}/></div>{canMutate && project.data && <section className="panel stack"><div><p className="eyebrow">Configuração</p><h2>Dados do Project</h2></div><NameEditor label="Nome do Project" initial={name} pending={update.isPending} error={update.isError ? userFacingError(update.error) : ""} onSave={(value) => update.mutateAsync(value)}/><div className="danger-zone"><div><strong>Arquivar Project</strong><p>Apps e Environments dependentes podem impedir esta ação.</p></div><ConfirmAction trigger="Arquivar" title={`Arquivar ${name}?`} description="O Project deixará de aparecer nas listas ativas. Dependências existentes podem bloquear a operação." confirmLabel="Arquivar Project" onConfirm={() => archive.mutateAsync()} pending={archive.isPending}/></div>{archive.isError && <Alert>{userFacingError(archive.error)}</Alert>}</section>}</>}
  </ProjectLayout>;
}
