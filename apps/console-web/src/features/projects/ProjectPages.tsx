...
export function ProjectsPage() {
  ...
  return <div className="stack"><PageHeader eyebrow="Estrutura" title="Projects" description="Agrupe Apps e Environments por produto ou iniciativa." actions={canMutate && <NameCreateForm label="Novo Project" button="Criar Project" pending={create.isPending} onCreate={(name) => create.mutateAsync(name)}/>}/>{create.isError && <Alert>{userFacingError(create.error)}</Alert>}{projects.isError && <Alert>{userFacingError(projects.error)}</Alert>}{projects.isPending ? <p className="muted" role="status">Carregando Projects…</p> : projects.isError ? null : projects.data?.items.length ? <div className="data-list">{projects.data.items.map((project) => <Link aria-label={`Abrir Project ${project.name}`} className="data-row" key={project.id} to="/workspaces/$workspaceId/projects/$projectId" params={{ workspaceId, projectId: project.id }}><span><strong>{project.name}</strong><small>{project.id}</small></span><span className="row-action">Abrir</span></Link>)}</div> : <EmptyState title="Nenhum Project" description="Crie um Project para organizar Apps e Environments."/>}</div>;
}
...
export function ProjectOverviewPage() {
  ...
  return <ProjectLayout workspaceId={workspaceId} projectId={projectId}>{(name) => <>{(apps.error || environments.error) && <Alert>{userFacingError(apps.error ?? environments.error)}</Alert>}<div className="summary-grid"><SummaryCard label="Apps no catálogo" value={apps.data?.items.length ?? 0} to="/workspaces/$workspaceId/projects/$projectId/settings/apps" params={{ workspaceId, projectId }}/><SummaryCard label="Environments" value={environments.data?.items.length ?? 0} to="/workspaces/$workspaceId/projects/$projectId/settings/environments" params={{ workspaceId, projectId }}/></div>{canMutate && project.data && <section className="panel stack"><div><p className="eyebrow">Configuração</p><h2>Dados do Project</h2></div><NameEditor label="Nome do Project" initial={name} pending={update.isPending} error={update.isError ? userFacingError(update.error) : ""} onSave={(value) => update.mutateAsync(value)}/><div className="danger-zone"><div><strong>Arquivar Project</strong><p>Apps e Environments dependentes podem impedir esta ação.</p></div><ConfirmAction trigger="Arquivar" title={`Arquivar ${name}?`} description="O Project deixará de aparecer nas listas ativas. Dependências existentes podem bloquear a operação." confirmLabel="Arquivar Project" onConfirm={() => archive.mutateAsync()} pending={archive.isPending}/></div>{archive.isError && <Alert>{userFacingError(archive.error)}</Alert>}</section>}</>}
  </ProjectLayout>;
}
