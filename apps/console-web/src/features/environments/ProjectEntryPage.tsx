...
export function ProjectEntryPage() {
  ...
  return <div className="stack"><PageHeader eyebrow="Project" title={project.data?.name ?? "Project"} description="Escolha um Environment para organizar os Apps em execução." breadcrumbs={[{ label: "Projects", to: "/workspaces/$workspaceId/projects", params: { workspaceId } }, { label: project.data?.name ?? "Project" }]}/><EmptyState title="Crie o primeiro Environment" description="Apps são configurados e operados dentro de um Environment." action={<Link className="button-link primary" to="/workspaces/$workspaceId/projects/$projectId/settings/environments" params={{ workspaceId, projectId }}>Configurar Environments</Link>}/></div>;
}
