...
export const RuntimeLogBody = memo(function RuntimeLogBody({ body }: { body: string }) {
  ...
  return <div className="runtime-log-body">
    <pre id={contentId} aria-label="Conteúdo JSON do log">{tokens.map((token, index) => <JsonToken key={index} token={token}/>)}</pre>
    <button type="button" className="runtime-log-json-toggle" aria-controls={contentId} aria-expanded={formatted} onClick={() => setFormatted((value) => !value)}>
