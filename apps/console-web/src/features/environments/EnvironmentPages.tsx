...
export function EnvironmentAppLayout({ children }: { children: (target: AppEnvironment, params: EnvironmentParams) => ReactNode }) {
  ...
  return <div className="stack"><PageHeader eyebrow={target.environmentName} title={target.appName} description={`${target.branch} · configuração desejada v${target.configurationVersion}`} breadcrumbs={[{ label: "Environment", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId", params: { workspaceId: params.workspaceId, projectId: params.projectId, environmentId: params.environmentId } }, { label: target.appName }]} actions={<StatusBadge status={target.state}/>}/><TabNav label="Áreas do App no Environment" items={[{ label: "Visão geral", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId", params: routeParams }, { label: "Entrega", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/deployments", params: routeParams }, { label: "Observabilidade", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/observability", params: routeParams }, { label: "Configuração", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/settings", params: routeParams }]}/>{children(target, params)}</div>;
}

function DeliveryNav({ params }: { params: EnvironmentParams }) {
  const routeParams = { workspaceId: params.workspaceId, projectId: params.projectId, environmentId: params.environmentId, appEnvironmentId: params.appEnvironmentId };
  return <TabNav label="Ciclo de entrega" items={[{ label: "Implantações", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/deployments", params: routeParams }, { label: "Builds", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/builds", params: routeParams }, { label: "Releases", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/releases", params: routeParams }]}/>;
}
...
export function EnvironmentAppObservabilityPage() {
  return <EnvironmentAppLayout>{(target, params) => <section className="stack"><div><p className="eyebrow">Operação</p><h2>Observabilidade</h2><p className="muted">A estrutura está preparada para logs, métricas e eventos correlacionados deste runtime.</p></div><Alert tone="info">O backend de observabilidade será conectado na fase 10. Nenhum dado sintético é exibido como telemetria real.</Alert><div className="summary-grid"><article className="summary-card static"><span>Logs de runtime</span><strong className="summary-text">Em preparação</strong><small>Consulta por tempo, instância e trace</small></article><article className="summary-card static"><span>Métricas</span><strong className="summary-text">Em preparação</strong><small>Saúde, latência e recursos</small></article><article className="summary-card static"><span>Eventos</span><strong className="summary-text">Em preparação</strong><small>Implantações e mudanças operacionais</small></article></div><section className="panel stack"><h3>Diagnóstico disponível hoje</h3><p className="muted">Logs de build continuam associados ao processo que produziu a Release, não ao runtime.</p><Link to="/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/builds" params={{ workspaceId: params.workspaceId, projectId: params.projectId, environmentId: params.environmentId, appEnvironmentId: target.id }}>Abrir logs de builds</Link></section></section>}</EnvironmentAppLayout>;
}
