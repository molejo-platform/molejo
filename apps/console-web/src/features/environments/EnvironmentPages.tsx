...
export function EnvironmentAppLayout({ children }: { children: (target: AppEnvironment, params: EnvironmentParams) => ReactNode }) {
  ...
  return <div className="stack"><PageHeader eyebrow={target.environmentName} title={target.appName} description={`${target.branch} · configuração desejada v${target.configurationVersion}`} breadcrumbs={[{ label: "Environment", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId", params: { workspaceId: params.workspaceId, projectId: params.projectId, environmentId: params.environmentId } }, { label: target.appName }]} actions={<StatusBadge status={target.state}/>}/><TabNav label="Áreas do App no Environment" items={[{ label: "Visão geral", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId", params: routeParams }, { label: "Entrega", to: deliveryPaths[0], params: routeParams, activeTo: deliveryPaths }, { label: "Observabilidade", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/observability", params: routeParams }, { label: "Configuração", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/settings", params: routeParams, activeTo: ["/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/settings"] }]}/>{children(target, params)}</div>;
}
