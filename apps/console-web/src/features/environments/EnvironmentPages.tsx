...
import { createAppBuild, createAppEnvironment, createAppEnvironmentDeployment, getAppBuild, getAppSource, listAppBuildLogs, listAppBuilds, listAppEnvironmentConfigurationVersions, listAppEnvironmentDeployments, listAppReleases, listStorageProfiles, previewAppEnvironmentDeployment } from "../apps/api";
...
function runtimeAddresses(configuration: RuntimeConfiguration) {
  return configuration.publicEndpoints.map((endpoint) => `${endpoint.hostnameLabel}.molejo.dev${endpoint.type === "TCP" && endpoint.externalPort ? `:${endpoint.externalPort}` : ""}`);
}
...
function AddAppToEnvironment({ workspaceId, projectId, environmentId, availableApps, onCreated }: { workspaceId: string; projectId: string; environmentId: string; availableApps: Array<{ id: string; name: string }>; onCreated: (target: AppEnvironment) => Promise<void> }) {
  ...
  const storageProfiles = useQuery({ queryKey: workspaceScopeKeys.storageProfiles(workspaceId), queryFn: () => listStorageProfiles(workspaceId) });
  ...
  useEffect(() => { if (!storageProfiles.data?.items.some((profile) => profile.id === storageProfileId)) setStorageProfileId(storageProfiles.data?.items[0]?.id ?? ""); }, [storageProfileId, storageProfiles.data?.items]);
  const create = useMutation({
    mutationFn: async () => {
      ...
      const runtime = { ...configuration, replicas: workloadKind === "Stateful" ? 1 : configuration.replicas, variables: parsedVariables.items };
  ...
  const statefulInvalid = workloadKind === "Stateful" && (!selectedProfile || sizeGiB < selectedProfile.minimumSizeGiB || sizeGiB > selectedProfile.maximumSizeGiB || sizeGiB > selectedProfile.availableGiB || !mountPath.trim().startsWith("/"));
  return <form className="panel stack" onSubmit={submit}><div><p className="eyebrow">Novo vínculo</p><h3>Adicionar App ao Environment</h3><p className="muted">O App organiza a fonte; este vínculo define branch e runtime neste Environment.</p></div><div className="choice-grid"><Button variant={mode === "existing" ? "primary" : "secondary"} type="button" onClick={() => setMode("existing")} disabled={!availableApps.length}>Usar App existente</Button><Button variant={mode === "new" ? "primary" : "secondary"} type="button" onClick={() => setMode("new")}>Criar novo App</Button></div>{mode === "existing" ? <SelectField label="App existente" value={appId} onChange={(event) => setAppId(event.target.value)} required><option value="">Selecione</option>{availableApps.map((app) => <option key={app.id} value={app.id}>{app.name}</option>)}</SelectField> : <Field label="Nome do novo App" value={name} onChange={(event) => { setName(event.target.value); setNameError(""); }} error={nameError} maxLength={80} required/>}<Field label="Branch" helper="Esta branch será construída para este Environment." value={branch} onChange={(event) => setBranch(event.target.value)} maxLength={255} required/><SelectField label="Tipo de execução" helper="Stateless não mantém arquivos locais. Stateful preserva um volume entre releases e recriações." value={workloadKind} onChange={(event) => { const next = event.target.value as "Stateless" | "Stateful"; setWorkloadKind(next); if (next === "Stateful") setConfiguration((current) => ({ ...current, replicas: 1 })); }} required><option value="Stateless">Stateless</option><option value="Stateful">Stateful</option></SelectField>{workloadKind === "Stateful" && <section className="review stack" aria-label="Armazenamento persistente"><div><strong>Armazenamento persistente</strong><p className="muted">Um único volume, uma réplica e retenção por padrão. O volume não é recriado ao trocar a release.</p></div>{storageProfiles.error && <Alert>{userFacingError(storageProfiles.error)}</Alert>}<SelectField label="Perfil de armazenamento" value={storageProfileId} onChange={(event) => setStorageProfileId(event.target.value)} disabled={storageProfiles.isPending} required><option value="">Selecione</option>{storageProfiles.data?.items.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} · até {profile.maximumSizeGiB} GiB</option>)}</SelectField>{selectedProfile && <p className="muted">{selectedProfile.availableGiB} GiB disponíveis. Expansão {selectedProfile.expandable ? "permitida" : "indisponível"}; snapshots e backup automático não fazem parte deste perfil.</p>}<div className="form-row"><Field label="Capacidade (GiB)" type="number" min={selectedProfile?.minimumSizeGiB ?? 1} max={Math.min(selectedProfile?.maximumSizeGiB ?? 1, selectedProfile?.availableGiB ?? 1)} value={sizeGiB} onChange={(event) => setSizeGiB(event.target.valueAsNumber)} required/><Field label="Caminho de montagem" helper="Diretório gravável usado pela aplicação, por exemplo /data." value={mountPath} onChange={(event) => setMountPath(event.target.value)} pattern="^/.+" maxLength={255} required/></div><Alert tone="warning">Neste laboratório, a disponibilidade dos dados acompanha a máquina de armazenamento. Backups automáticos ainda não estão incluídos.</Alert></section>}{parameters.error && <Alert>{userFacingError(parameters.error)}</Alert>}<RuntimeConfigurationFields value={configuration} onChange={setConfiguration} variables={variables} onVariablesChange={(value) => { setVariables(value); setVariablesError(""); }} variablesError={variablesError} availableParameters={parameters.data?.items}/>{createdWithoutTarget && create.isError && <Alert tone="warning">O App {createdWithoutTarget} foi criado, mas o vínculo falhou. Tente novamente usando “App existente”.</Alert>}{create.isError && !variablesError && !nameError && <Alert>{userFacingError(create.error)}</Alert>}<div className="form-actions"><Button type="submit" loading={create.isPending} disabled={!branch.trim() || statefulInvalid || (mode === "existing" ? !appId : !name.trim())}>{mode === "new" ? "Criar e adicionar" : "Adicionar ao Environment"}</Button></div></form>;
}
...
export function EnvironmentAppLayout({ children }: { children: (target: AppEnvironment, params: EnvironmentParams) => ReactNode }) {
  ...
  const settingsBase = "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId/settings";
  const settingsPaths = [settingsBase, `${settingsBase}/build`, `${settingsBase}/secrets`, `${settingsBase}/network`, `${settingsBase}/health`, `${settingsBase}/resources`, `${settingsBase}/storage`, `${settingsBase}/versions`] as const;
  return <RuntimeMetricsProvider target={target} params={params}><div className="stack"><PageHeader eyebrow={target.environmentName} title={target.appName} description={`${target.workloadKind} · ${target.branch} · configuração desejada v${target.configurationVersion}`} breadcrumbs={[{ label: "Environment", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId", params: { workspaceId: params.workspaceId, projectId: params.projectId, environmentId: params.environmentId } }, { label: target.appName }]} actions={<StatusBadge status={target.state}/>}/><RuntimeStatusStrip target={target}/><TabNav label="Áreas do App no Environment" items={[{ label: "Visão geral", to: "/workspaces/$workspaceId/projects/$projectId/environments/$environmentId/apps/$appEnvironmentId", params: routeParams }, { label: "Entrega", to: deliveryPaths[0], params: routeParams, activeTo: deliveryPaths }, { label: "Observabilidade", to: observabilityPaths[0], params: routeParams, activeTo: observabilityPaths }, { label: "Configuração", to: settingsBase, params: routeParams, activeTo: settingsPaths }]}/>{children(target, params)}</div></RuntimeMetricsProvider>;
}
...
function TargetOverview({ target }: { target: AppEnvironment }) {
  ...
  const addresses = runtimeAddresses(target.configuration);
  return <section className="stack">{target.message && <Alert tone={target.state === "Degraded" ? "error" : "info"}>{target.message}</Alert>}{pendingConfiguration && <Alert tone="warning">Configuração v{target.configurationVersion} salva, ainda não implantada.</Alert>}{operationActive && <Alert tone="info">Uma implantação está em andamento. Novas ações ficam bloqueadas até sua conclusão.</Alert>}<section className="panel stack"><div><p className="eyebrow">Runtime</p><h2>Resumo operacional</h2></div><dl className="detail-grid"><div><dt>Tipo de execução</dt><dd>{target.workloadKind}</dd></div><div><dt>Branch de build</dt><dd className="mono">{target.branch}</dd></div><div><dt>Endereços públicos</dt><dd>{addresses.join(", ") || "Acesso privado"}</dd></div><div><dt>Portas internas</dt><dd>{target.configuration.ports.map((port) => `${port.name}:${port.containerPort}`).join(", ")}</dd></div><div><dt>Réplicas desejadas</dt><dd>{target.configuration.replicas}</dd></div><div><dt>Deployment corrente</dt><dd className="mono">{target.currentDeploymentId ?? "—"}</dd></div><div><dt>Última atualização</dt><dd>{formatDateTime(target.updatedAt)}</dd></div></dl></section></section>;
}
...
function TargetBuildDetail({ target, params }: { target: AppEnvironment; params: EnvironmentParams }) {
  ...
  return <section className="stack"><div className="section-heading"><div><p className="eyebrow">Build</p><h2>{build.data.commitTitle || shortSha(build.data.commitSha)}</h2><p className="muted">{build.data.repository} · {build.data.branch}</p></div><Button variant="icon" aria-label="Atualizar build" onClick={() => { void build.refetch(); void logs.refetch(); }}><Icon name="refresh"/></Button></div><section className="panel stack"><StatusBadge status={build.data.status}/><dl className="detail-grid"><div><dt>SHA</dt><dd className="mono">{build.data.commitSha}</dd></div><div><dt>Autor</dt><dd>{build.data.commitAuthorLogin || build.data.commitAuthorName || "—"}</dd></div><div><dt>Origem</dt><dd>{build.data.trigger}</dd></div><div><dt>Plataforma</dt><dd>{build.data.platform}</dd></div><div><dt>Tentativas</dt><dd>{build.data.attempts}</dd></div><div><dt>Atualizado</dt><dd>{formatDateTime(build.data.updatedAt)}</dd></div></dl>{build.data.errorMessage && <Alert>{build.data.errorMessage}</Alert>}</section><section className="panel stack"><div><p className="eyebrow">Diagnóstico</p><h2>Logs sanitizados</h2></div>{logs.isPending ? <p className="muted" role="status">Carregando logs…</p> : logs.isError ? <Alert>{userFacingError(logs.error)}</Alert> : logs.data?.items.length ? <pre className="build-logs" aria-label="Logs do build">{logs.data.items.map((item) => item.message).join("\n")}</pre> : <EmptyState title="Logs ainda indisponíveis" description="O worker ainda não registrou saída para este build."/>}</section></section>;
}
...
function TargetDeployments({ target, params, canMutate, queryClient }: { target: AppEnvironment; params: EnvironmentParams; canMutate: boolean; queryClient: ReturnType<typeof useQueryClient> }) {
  ...
  const availableReleases = useMemo(() => releases.data?.items.filter((release) => release.appEnvironmentId === target.id && release.availabilityStatus === "Available") ?? [], [releases.data?.items, target.id]);
  ...
  useEffect(() => { if (!availableReleases.some((release) => release.id === releaseId)) setReleaseId(availableReleases[0]?.id ?? ""); }, [availableReleases, releaseId]);
...
function TargetReleases({ target, params, canMutate, queryClient }: { target: AppEnvironment; params: EnvironmentParams; canMutate: boolean; queryClient: ReturnType<typeof useQueryClient> }) {
  ...
  const rebuild = useMutation({ mutationFn: (commitSha: string) => createAppBuild(params.workspaceId, params.projectId, target.appId, { appEnvironmentId: target.id, commitSha }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: workspaceScopeKeys.appBuilds(params.workspaceId, params.projectId, target.appId) }); } });
  ...
  return <section className="stack"><DeliveryNav params={params}/><div><p className="eyebrow">Artefatos</p><h2>Releases</h2><p className="muted">Este App no Environment mantém até três imagens disponíveis; o histórico permanece visível para reconstrução.</p></div>{rebuild.isSuccess && <Alert tone="success">Reconstrução enfileirada para este commit.</Alert>}{releases.isPending ? <p className="muted" role="status">Carregando releases…</p> : items.length ? <div className="data-list">{items.map((release) => <div className="data-row" key={release.id}><span><strong>{release.commitTitle || shortSha(release.commitSha)}</strong><small><span className="mono">{shortSha(release.commitSha)}</span> · {release.commitAuthorLogin || release.commitAuthorName || "autor indisponível"} · {release.trigger} · {formatDateTime(release.createdAt)}</small></span><span className="row-action">{release.availabilityStatus === "Expired" && canMutate ? <Button type="button" variant="secondary" loading={rebuild.isPending} onClick={() => rebuild.mutate(release.commitSha)}>Reconstruir este commit</Button> : release.availabilityStatus === "Expired" ? "Expirada" : "Disponível"}</span></div>)}</div> : <EmptyState title="Nenhuma release" description="Um build bem-sucedido criará o primeiro artefato implantável."/>}</section>;
}
