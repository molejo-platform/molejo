...
export function publicationDomains(session: Session | null | undefined, workloadKind: "Stateless" | "Stateful", endpointType: Endpoint["type"]): PublicationDomain[] {
  return (session?.installationCapabilities.publicationDomains ?? []).filter((domain) => domain.workloadKinds.includes(workloadKind) && domain.endpointTypes.includes(endpointType));
}

export function publicationSuffix(session: Session | null | undefined, domainId: string) {
  return session?.installationCapabilities.publicationDomains?.find((domain) => domain.id === domainId)?.suffix ?? "molejo.dev";
}
