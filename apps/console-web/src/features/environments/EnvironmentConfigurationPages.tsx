...
export const EnvironmentNetworkPage = page("network", "Rede", "Defina portas internas nomeadas e publique, no máximo, um endpoint HTTP e um TCP.", (draft, setDraft, _parameters, disabled) => <NetworkEditor draft={draft} setDraft={setDraft} disabled={disabled}/>);

function NetworkEditor({ draft, setDraft, disabled }: { draft: RuntimeConfiguration; setDraft: (value: RuntimeConfiguration) => void; disabled: boolean }) {
  const session = useSessionQuery();
  ...
  return <section className="stack"><div className="section-heading"><div><strong>Portas internas</strong><p className="muted">Nomes estáveis conectam Service, probes e publicação sem expor objetos Kubernetes.</p></div>{!disabled && <Button type="button" variant="secondary" onClick={addPort} disabled={draft.ports.length >= 8}>Adicionar porta</Button>}</div>{draft.ports.map((port, index) => <div className="form-row" key={`${port.name}-${index}`}><Field label="Nome" value={port.name} onChange={(event) => updatePort(index, { name: event.target.value })} pattern="^[a-z0-9](?:[-a-z0-9]*[a-z0-9])?$" maxLength={15} disabled={disabled} required/><Field label="Porta no container" type="number" min={1} max={65535} value={port.containerPort} onChange={(event) => updatePort(index, { containerPort: event.target.valueAsNumber })} disabled={disabled} required/>{!disabled && draft.ports.length > 1 && <Button type="button" variant="secondary" onClick={() => removePort(port.name)}>Remover</Button>}</div>)}<div className="form-row"><SelectField label="HTTP público" value={http ? "enabled" : "disabled"} onChange={(event) => replaceEndpoint("HTTP", event.target.value === "enabled" ? { name: "web", type: "HTTP", portName: draft.ports[0].name, hostnameLabel: "app" } : undefined)} disabled={disabled}><option value="disabled">Desativado</option><option value="enabled">Ativado</option></SelectField>{http && <><SelectField label="Porta HTTP" value={http.portName} onChange={(event) => replaceEndpoint("HTTP", { ...http, portName: event.target.value })} disabled={disabled}>{draft.ports.map((port) => <option key={port.name} value={port.name}>{port.name} · {port.containerPort}</option>)}</SelectField><Field label="Hostname HTTP" helper={`${http.hostnameLabel || "app"}.molejo.dev`} value={http.hostnameLabel} onChange={(event) => replaceEndpoint("HTTP", { ...http, hostnameLabel: event.target.value })} pattern="^[a-z0-9](?:[-a-z0-9]*[a-z0-9])?$" maxLength={63} disabled={disabled} required/></>}</div>{session.data?.installationCapabilities?.publicTCP?.enabled ? <div className="form-row"><SelectField label="TCP público (experimental)" value={tcp ? "enabled" : "disabled"} onChange={(event) => replaceEndpoint("TCP", event.target.value === "enabled" ? { name: "tcp", type: "TCP", portName: draft.ports[0].name, hostnameLabel: "app-tcp" } : undefined)} disabled={disabled}><option value="disabled">Desativado</option><option value="enabled">Ativado</option></SelectField>{tcp && <><SelectField label="Porta TCP" value={tcp.portName} onChange={(event) => replaceEndpoint("TCP", { ...tcp, portName: event.target.value, externalPort: undefined })} disabled={disabled}>{draft.ports.map((port) => <option key={port.name} value={port.name}>{port.name} · {port.containerPort}</option>)}</SelectField><Field label="Hostname TCP" helper={`${tcp.hostnameLabel || "app-tcp"}.molejo.dev${tcp.externalPort ? `:${tcp.externalPort}` : " · porta alocada ao salvar"}`} value={tcp.hostnameLabel} onChange={(event) => replaceEndpoint("TCP", { ...tcp, hostnameLabel: event.target.value, externalPort: undefined })} pattern="^[a-z0-9](?:[-a-z0-9]*[a-z0-9])?$" maxLength={63} disabled={disabled} required/></>}</div> : <Alert tone="info">A publicação TCP experimental não está habilitada nesta instalação.</Alert>}</section>;
}
