...
export const createParameter = (workspaceId: string, input: ParameterInput) => request<Parameter>(base(workspaceId), { method: "POST", body: JSON.stringify(input) });
export const replaceParameter = (workspaceId: string, parameter: Parameter, input: ParameterInput) => request<Parameter>(`${base(workspaceId)}/${encodeURIComponent(parameter.id)}`, { method: "PUT", headers: { "If-Match": String(parameter.version) }, body: JSON.stringify(input) });
