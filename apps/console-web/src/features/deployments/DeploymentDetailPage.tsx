  ...
  return (
    <>
      ...
      <section className="card">
        <div className="section-heading"><div><p className="eyebrow">Detalhe</p><h2>{current.intent.name}</h2></div>{session.data?.actor.role === "owner" && <div className="topbar-actions"><Link className="secondary button-link" to="/deployments/$deploymentId/edit" params={{ deploymentId: current.id }}>Editar</Link><DeleteDeploymentDialog deployment={current} /></div>}</div>
