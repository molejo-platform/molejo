...
import { getAppSource, listAppBuilds, listAppEnvironments, listAppReleases } from "./api";
...
export function AppOverviewPage() {
  ...
  const error = source.error ?? targets.error ?? builds.error ?? releases.error;
  const params = { workspaceId, projectId, appId };
  return <AppLayout workspaceId={workspaceId} projectId={projectId} appId={appId}>{() => <>{error && <Alert>{userFacingError(error)}</Alert>}<div className="summary-grid"><Link className="summary-card" to="/workspaces/$workspaceId/projects/$projectId/apps/$appId/source" params={params}><span>Fonte</span><strong className="summary-text">{source.data?.source ? source.data.source.repository.fullName : "Não configurada"}</strong><small>Selecionar repositório</small></Link><Link className="summary-card" to="/workspaces/$workspaceId/projects/$projectId/apps/$appId/environments" params={params}><span>App Environments</span><strong>{targets.data?.items.length ?? "—"}</strong><small>Branches e runtime</small></Link><Link className="summary-card" to="/workspaces/$workspaceId/projects/$projectId/apps/$appId/builds" params={params}><span>Builds</span><strong>{builds.data?.items.length ?? "—"}</strong><small>Executar e acompanhar</small></Link><Link className="summary-card" to="/workspaces/$workspaceId/projects/$projectId/apps/$appId/releases" params={params}><span>Releases</span><strong>{releases.data?.items.length ?? "—"}</strong><small>Consultar artefatos</small></Link></div><section className="panel"><p className="eyebrow">Próxima ação</p><h2>{!source.data?.source ? "Configure a fonte" : !targets.data?.items.length ? "Configure um App Environment" : !releases.data?.items.length ? "Produza a primeira release" : "Release disponível"}</h2><p className="muted">{!source.data?.source ? "Selecione um repositório autorizado pela instalação GitHub do Workspace." : !targets.data?.items.length ? "Associe o App a um Environment e defina sua branch e runtime." : !releases.data?.items.length ? "Inicie um build para um App Environment e acompanhe o processamento." : "Implante uma release em um App Environment existente."}</p></section></>}</AppLayout>;
}
