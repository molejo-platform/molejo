...
export function AppReleasesPage() {
  ...
  return <AppLayout workspaceId={workspaceId} projectId={projectId} appId={appId}>{() => <section className="stack"><div><p className="eyebrow">Artefatos</p><h2>Releases</h2><p className="muted">Cada release associa o commit construído a uma imagem OCI imutável por digest.</p></div>{releases.isError && <Alert>{userFacingError(releases.error)}</Alert>}{releases.isPending ? <p className="muted" role="status">Carregando releases…</p> : releases.data?.items.length ? <div className="data-list">{releases.data.items.map((release) => <div className="data-row" key={release.id}><span><strong className="mono">{shortSha(release.commitSha)}</strong><small>{release.platform} · {formatDateTime(release.createdAt)}</small><small className="digest">{release.image}</small></span></div>)}</div> : <EmptyState title="Nenhuma release" description="Uma release será criada quando um build for concluído com sucesso." action={<Link className="secondary button-link" to="/workspaces/$workspaceId/projects/$projectId/apps/$appId/builds" params={{ workspaceId, projectId, appId }}>Ver builds</Link>}/>}</section>}</AppLayout>;
}
