...
export function AppSourcePage() {
  ...
  useEffect(() => { const current = source.data?.source?.installationId === installationId ? source.data.source.repository.id : ""; const next = repositories.data?.items.some((item) => item.id === current) ? current : repositories.data?.items[0]?.id ?? ""; setRepositoryId(next); }, [installationId, repositories.data?.items, source.data?.source]);
  ...
  }, onSuccess: () => queryClient.invalidateQueries({ queryKey: workspaceScopeKeys.appSource(workspaceId, projectId, appId) }) });
  ...
  return <AppLayout workspaceId={workspaceId} projectId={projectId} appId={appId}>{(appName) => <section className="panel stack"><div><p className="eyebrow">Repositório</p><h2>Fonte do App</h2><p className="muted">O App seleciona um repositório. Cada App Environment define sua própria branch.</p></div>{error && <Alert>{userFacingError(error)}</Alert>}{source.data?.source && <Alert tone="success">Fonte atual: <strong>{source.data.source.repository.fullName}</strong>.</Alert>}{installations.isPending ? <p className="muted" role="status">Carregando instalações…</p> : installations.isError ? null : installations.data?.items.length ? <div className="stack"><div className="form-row"><SelectField label="Instalação GitHub" value={selectedInstallation?.id ?? ""} onChange={(event) => { setRepositoryId(""); setInstallationId(event.target.value); }} disabled={!canMutate}>{installations.data.items.map((installation) => <option key={installation.id} value={installation.id}>{installation.accountLogin}</option>)}</SelectField><SelectField label="Repositório" helper="O mesmo repositório pode alimentar Apps diferentes." value={repositoryId} onChange={(event) => setRepositoryId(event.target.value)} disabled={!canMutate || repositories.isPending}>{repositories.data?.items.map((repository) => <option key={repository.id} value={repository.id}>{repository.fullName}{repository.private ? " · privado" : ""}</option>)}</SelectField></div>{canMutate && <div className="form-actions"><Button onClick={() => save.mutate()} loading={save.isPending} disabled={repositories.isPending || !repositoryValid}>Salvar fonte</Button>{source.data?.source && <ConfirmAction trigger="Remover fonte" title={`Remover a fonte de ${appName}?`} description="Novos builds ficarão bloqueados até que outro repositório seja selecionado. Builds e releases existentes permanecem disponíveis." confirmLabel="Remover fonte" onConfirm={() => clear.mutateAsync()} pending={clear.isPending}/>}</div>}</div> : <EmptyState title="Conecte o GitHub primeiro" description="Este Workspace ainda não possui uma instalação GitHub autorizada." action={<Link className="button-link primary" to="/workspaces/$workspaceId/settings/github" params={{ workspaceId }}>Abrir integração GitHub</Link>}/>}</section>}</AppLayout>;
}
