...
export function AppLayout({ workspaceId, projectId, appId, children }: { workspaceId: string; projectId: string; appId: string; children: (appName: string) => ReactNode }) {
  ...
  return <div className="stack"><PageHeader eyebrow="App" title={app.data.name} breadcrumbs={[{ label: "Projects", to: "/workspaces/$workspaceId/projects", params: { workspaceId } }, { label: project.data.name, to: "/workspaces/$workspaceId/projects/$projectId", params: { workspaceId, projectId } }, { label: app.data.name }]}/><TabNav label="Ciclo do App" items={[{ label: "Visão geral", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId", params }, { label: "Fonte", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId/source", params }, { label: "Environments", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId/environments", params }, { label: "Builds", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId/builds", params }, { label: "Releases", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId/releases", params }]}/>{children(app.data.name)}</div>;
}
