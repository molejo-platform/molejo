...
export function AppLayout({ workspaceId, projectId, appId, children }: { workspaceId: string; projectId: string; appId: string; children: (appName: string) => ReactNode }) {
  ...
  return <div className="stack"><PageHeader eyebrow="Configuração do App" title={app.data.name} description="Definição compartilhada entre Environments." breadcrumbs={[{ label: "Projects", to: "/workspaces/$workspaceId/projects", params: { workspaceId } }, { label: project.data.name, to: "/workspaces/$workspaceId/projects/$projectId", params: { workspaceId, projectId } }, { label: "Catálogo de Apps", to: "/workspaces/$workspaceId/projects/$projectId/settings/apps", params: { workspaceId, projectId } }, { label: app.data.name }]}/><TabNav label="Configuração do App" items={[{ label: "Visão geral", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId", params }, { label: "Fonte", to: "/workspaces/$workspaceId/projects/$projectId/apps/$appId/source", params }]}/>{children(app.data.name)}</div>;
}
