...
vi.mock("../auth/model", () => ({
  useSessionQuery: () => ({ data: { user: { id: "usr-administrator" }, installationCapabilities: { manageUsers: true } } }),
}));
vi.mock("./api", () => ({
  identityKeys: { users: ["identity", "users"], installationAudit: ["identity", "installation-audit"] },
...
describe("installation user management", () => {
  it("creates an invitation without asking the administrator for an initial password", async () => {
    ...
    mocks.createUser.mockResolvedValue({
      user: { id: "usr-invited", username: "new.user", displayName: "New User", status: "Invited", installationAdministrator: false, version: 1, createdAt: "2026-09-06T12:00:00Z", updatedAt: "2026-09-06T12:00:00Z" },
    ...
    await waitFor(() => expect(mocks.createUser).toHaveBeenCalledWith({ username: "new.user", displayName: "New User", installationAdministrator: false }));
