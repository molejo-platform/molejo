...
describe("invitation activation", () => {
  it("submits the one-time token and user-selected password before returning to login", async () => {
    ...
    render(<QueryClientProvider client={queryClient}><AcceptInvitationPage /></QueryClientProvider>);
    ...
    await waitFor(() => expect(mocks.acceptUserInvitation).toHaveBeenCalledWith("one-time-invitation-token", "a durable user password"));
