...
import type { AccessGrant, AuditEvent, EffectiveCapabilities, InstallationUser, MFAStatus, TOTPEnrollment, User, UserInvitation, UserSession, WorkspaceGroup, WorkspaceMembership } from "../../shared/api/types";

export const identityKeys = {
  users: ["identity", "users"] as const,
  installationAudit: ["identity", "installation-audit"] as const,
  ...
  capabilities: (workspaceId: string, resourceType: string, resourceId: string) => ["workspaces", workspaceId, "capabilities", resourceType, resourceId] as const,
};
...
export function listUsers(cursor?: string) { return request<{ items: InstallationUser[]; nextCursor: string | null }>(`/api/v1/admin/users?limit=50${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`); }
export function createUser(input: { username: string; displayName: string; installationAdministrator: boolean }) { return request<UserInvitation>("/api/v1/admin/users", { method: "POST", body: JSON.stringify(input) }); }
export function updateUserStatus(user: InstallationUser, status: "Active" | "Locked" | "Disabled") { return request<InstallationUser>(`/api/v1/admin/users/${user.id}`, { method: "PUT", headers: { "If-Match": String(user.version) }, body: JSON.stringify({ status }) }); }
export function updateInstallationRole(user: InstallationUser, administrator: boolean) { return request<InstallationUser>(`/api/v1/admin/users/${user.id}/installation-role`, { method: "PUT", headers: { "If-Match": String(user.version) }, body: JSON.stringify({ administrator }) }); }
export function createUserInvitation(userId: string) { return request<{ token: string; expiresAt: string }>(`/api/v1/admin/users/${userId}/invitation`, { method: "POST" }); }
...
export function listInstallationAudit() { return request<{ items: AuditEvent[]; nextCursor: string | null }>("/api/v1/admin/audit-events?limit=50"); }
export function acceptUserInvitation(token: string, password: string) { return request<void>("/api/v1/user-invitations/accept", { method: "PUT", body: JSON.stringify({ token, password }) }); }
...
export function getEffectiveCapabilities(workspaceId: string, resourceType: "Workspace" | "Project" | "App" | "AppEnvironment", resourceId: string) { return request<EffectiveCapabilities>(`/api/v1/workspaces/${workspaceId}/authorization/capabilities?resourceType=${resourceType}&resourceId=${encodeURIComponent(resourceId)}`); }
