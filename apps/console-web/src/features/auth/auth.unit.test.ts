...
import { sessionQueryOptions } from "./model";
...
describe("auth slice", () => {
  ...
  it("does not recover or retry a domain error that uses HTTP 401", async () => {
    const fetchMock = vi.mocked(fetch);
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ code: "current_password_invalid", message: "current password is invalid", requestId: "req-1" }), { status: 401 }));
    const recover = vi.fn(async () => setCsrfToken("rotated"));
    setCsrfRecovery(recover);

    await expect(request<void>("/api/v1/users/me/password", { method: "PUT", body: JSON.stringify({ currentPassword: "wrong", newPassword: "a sufficiently long password" }) })).rejects.toMatchObject({
      status: 401,
