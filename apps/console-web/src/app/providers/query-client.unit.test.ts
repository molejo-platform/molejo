...
import { sessionQueryKey } from "../../shared/auth/session-state";
  ...
  it("preserves authenticated data for a non-session 401", async () => {
    ...
    await expect(
      queryClient.fetchQuery({
        ...
        queryFn: () =>
          Promise.reject(
            new ApiRequestError(401, { code: "current_password_invalid", message: "invalid", requestId: "req-1" }),
          ),
      }),
  ...
  it("clears authenticated data for an explicit unauthenticated response", async () => {
    ...
    await expect(
      queryClient.fetchQuery({
        queryKey: ["expired"],
        queryFn: () =>
          Promise.reject(new ApiRequestError(401, { code: "unauthenticated", message: "expired", requestId: "req-1" })),
      }),
    ).rejects.toBeInstanceOf(ApiRequestError);
