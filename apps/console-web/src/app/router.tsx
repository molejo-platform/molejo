import { Outlet, createRootRouteWithContext, createRoute, createRouter, createBrowserHistory, redirect, useRouter } from "@tanstack/react-router";
...
import { SessionLoading, SessionUnavailable } from "../features/auth/SessionBoundary";
...
import { AcceptInvitationPage, AccountPage, ForgotPasswordPage } from "../features/identity/AccountPages";
...
const acceptInvitationRoute = createRoute({ getParentRoute: () => rootRoute, path: "/accept-invitation", component: AcceptInvitationPage });
...
async function requireInstallationCapability(context: RouterContext, capability: "manageUsers" | "createWorkspace") {
  const session = await context.queryClient.ensureQueryData(sessionQueryOptions());
...
const newWorkspaceRoute = createRoute({ getParentRoute: () => protectedRoute, path: "/workspaces/new", beforeLoad: ({ context }) => requireInstallationCapability(context, "createWorkspace"), component: NewWorkspacePage });
const accountRoute = createRoute({ getParentRoute: () => protectedRoute, path: "/account", component: AccountPage });
const administrationRoute = createRoute({ getParentRoute: () => protectedRoute, path: "/admin/users", beforeLoad: ({ context }) => requireInstallationCapability(context, "manageUsers"), component: AdministrationPage });

const routeTree = rootRoute.addChildren([
  loginRoute, forgotPasswordRoute, acceptInvitationRoute,
