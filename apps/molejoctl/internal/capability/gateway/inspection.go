...
func InspectConsumption(ctx context.Context, reader kubernetespublication.Reader, destination kubernetesbinding.HTTPDestination, hostname, consumerNamespace string) (kubernetespublication.Facts, error) {
	return kubernetespublication.Inspect(ctx, reader, kubernetespublication.Target{GatewayNamespace: destination.GatewayNamespace, GatewayName: destination.GatewayName, SectionName: destination.SectionName, Hostname: hostname, ConsumerNamespace: consumerNamespace})
}
