...
func Load(path string) (Setup, error) {
	...
	if err != nil {
		return Setup{}, fmt.Errorf("read gateway setup: %w", err)
	}
	var setup Setup
	if err = yaml.Unmarshal(contents, &setup); err != nil {
		return Setup{}, fmt.Errorf("decode gateway setup: %w", err)
	}
	return setup, nil
}
...
func Encode(setup Setup) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode gateway setup: %w", err)
	}
	return contents, nil
}
...
func diagnosticsError(diagnostics []Diagnostic) error {
	...
	return fmt.Errorf("invalid gateway setup: %s", message)
}
