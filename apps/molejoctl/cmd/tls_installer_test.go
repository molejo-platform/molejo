...
type fakeTLSEnvironment struct {
	initial    clustertls.Facts
	ready      clustertls.Facts
	operations []clustertls.Operation
}

func (f *fakeTLSEnvironment) Discover(context.Context, clustertls.Setup) (clustertls.Facts, error) {
...
func TestTLSPrepareConvergesAndRequiresApproval(t *testing.T) {
	...
	readyFacts := clustertls.Facts{
		Certificate: clustertls.CertificateFacts{Exists: true, Valid: true, KeyMatches: true, DNSNamesCovered: true, NotBefore: now, NotAfter: now.Add(90 * 24 * time.Hour)},
