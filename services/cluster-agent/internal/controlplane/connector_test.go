...
func TestControlChannelHelloHasBoundedWait(t *testing.T) {
	...
	err := runControlChannel(ctx, stream, "agi-abcdefghijklmnopqrst", "trust-v1", "test", AgentMetadata{}, nil, nil, nil, nil, nil, 20*time.Millisecond)
...
func TestControlChannelHeartbeatHasBoundedWait(t *testing.T) {
	...
	err := runControlChannel(ctx, stream, "agi-abcdefghijklmnopqrst", "trust-v1", "test", AgentMetadata{}, nil, nil, nil, func() { paired = true }, nil, 20*time.Millisecond)
...
func TestControlChannelRequestsTrustBundleRenewalBeforeHeartbeat(t *testing.T) {
	...
	defer cancel()
	stream := &blockingAgentStream{ctx: ctx}
	if err := runControlChannel(ctx, stream, "agi-abcdefghijklmnopqrst", "trust-old", "test", AgentMetadata{}, nil, nil, nil, nil, nil, 20*time.Millisecond); !errors.Is(err, agentidentity.ErrTrustBundleUpdateRequired) {
