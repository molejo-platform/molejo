...
func TestSecretStorePersistsIdentityAndClearsEnrollmentToken(t *testing.T) {
	client := fake.NewClientset(
		&corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "molejo-agent-identity", Namespace: "fruto-system"}},
		&corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "molejo-agent-enrollment", Namespace: "fruto-system"}, Data: map[string][]byte{"token": []byte("one-time")}},
	)
	store := NewSecretStore(client.CoreV1(), "fruto-system", "molejo-agent-identity", "molejo-agent-enrollment")
