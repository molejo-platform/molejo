...
type DeliveryStore interface {
	...
	CreateDeployment(context.Context, int64, domain.DeploymentRequest, audit.Event) (domain.Deployment, domain.Operation, bool, error)
}
...
func (w Worker) RunOnce(ctx context.Context, workerID string) (bool, error) {
	delivery, ok, err := w.Store.ClaimNextGitHubDelivery(ctx, workerID, w.lease())
	if err != nil || !ok {
		return ok, err
	}
	if delivery.EventType == "ping" {
		return true, w.Store.CompleteGitHubDelivery(ctx, delivery, true)
	}
	if delivery.EventType == "installation" || delivery.EventType == "installation_repositories" {
		if err = w.Store.ApplyGitHubInstallationDelivery(ctx, delivery); err != nil {
			return true, w.Store.FailGitHubDelivery(ctx, delivery, "installation_update_failed", "GitHub installation state could not be updated", true)
		}
		return true, w.Store.CompleteGitHubDelivery(ctx, delivery, false)
	}
	...
	if err != nil {
		...
		return true, w.Store.FailGitHubDelivery(ctx, delivery, "commit_unavailable", "GitHub commit could not be resolved", retryable)
	}
	if trigger == domain.TriggerPush && metadata.SHA != delivery.CommitSHA {
		return true, w.Store.FailGitHubDelivery(ctx, delivery, "commit_mismatch", "GitHub commit did not match the signed delivery", false)
	}
	candidates, err := w.Store.DeliveryCandidates(ctx, delivery, trigger)
	if err != nil {
		return true, w.Store.FailGitHubDelivery(ctx, delivery, "target_resolution_failed", "delivery targets could not be resolved", true)
	}
	for _, candidate := range candidates {
		if err = w.createTarget(ctx, delivery, candidate, metadata, trigger); err != nil {
			retryable := !errors.Is(err, store.ErrNotFound)
			return true, w.Store.FailGitHubDelivery(ctx, delivery, "target_schedule_failed", "delivery target could not be scheduled", retryable)
		}
	}
	return true, w.Store.CompleteGitHubDelivery(ctx, delivery, len(candidates) == 0)
}
...
func (w Worker) createTarget(ctx context.Context, delivery domain.GitHubDelivery, candidate domain.DeliveryCandidate, metadata domain.CommitMetadata, trigger string) error {
	for range 3 {
		...
		_, _, err = w.Store.CreateDeliveryTargetBuild(ctx, delivery, candidate, metadata, trigger, targetID, buildID)
...
func (w Worker) AdvanceOnce(ctx context.Context) error {
	targets, err := w.Store.DeliveryTargetsToAdvance(ctx, 50)
	if err != nil {
		return err
	}
	for _, target := range targets {
		...
		case "Building":
			...
			case domain.BuildSucceeded:
				err = w.Store.MarkDeliveryTarget(ctx, target.ID, "DeployPending", "", "")
			case domain.BuildFailed, domain.BuildTimedOut:
				err = w.Store.MarkDeliveryTarget(ctx, target.ID, "Failed", "build_failed", "automated build did not succeed")
			case domain.BuildSuperseded:
				err = w.Store.MarkDeliveryTarget(ctx, target.ID, "Superseded", "", "")
			}
		case "DeployPending":
		...
		case "Deploying":
			...
			case domain.Ready:
				err = w.Store.MarkDeliveryTarget(ctx, target.ID, "Succeeded", "", "")
			case domain.Degraded:
				err = w.Store.MarkDeliveryTarget(ctx, target.ID, "Failed", "deployment_degraded", "automated deployment became degraded")
			}
		}
...
func (w Worker) createDeployment(ctx context.Context, target domain.DeliveryTarget) error {
	appEnvironment, err := w.Store.FindAppEnvironment(ctx, target.WorkspaceID, target.AppEnvironmentPublicID)
	if err != nil {
		return err
	}
	...
	deployment, _, _, err := w.Store.CreateDeployment(ctx, target.RequestedByActorID, domain.DeploymentRequest{
		WorkspaceID:                       target.WorkspaceID,
		AppEnvironmentPublicID:            target.AppEnvironmentPublicID,
		DeploymentPublicID:                deploymentID,
		ReleasePublicID:                   target.ReleasePublicID,
		ConfigurationVersion:              appEnvironment.ConfigurationVersion,
		ExpectedVersion:                   appEnvironment.Version,
		ExpectedCurrentDeploymentPublicID: appEnvironment.CurrentDeploymentPublicID,
	...
	return w.Store.AttachDeliveryDeployment(ctx, target.ID, deployment.ID)
}
...
func (w Worker) Run(ctx context.Context, workerID string) {
	defer func() {
		...
		defer cancel()
		_ = w.Store.ReleaseGitHubDeliveryClaims(releaseCtx, workerID)
	}()
