...
func configureAgentPairing(registry *store.Store, dispatcher controlagent.RuntimeDispatcher) (api.AgentCertificateSigner, *grpc.Server, net.Listener, error) {
	...
	if provided != len(configured) {
		return nil, nil, nil, fmt.Errorf("Agent pairing TLS configuration is incomplete")
	}
	caCertificatePEM, err := os.ReadFile(caCertificatePath)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("read Agent CA certificate: %w", err)
	}
	caKeyPEM, err := os.ReadFile(caKeyPath)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("read Agent CA key: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("load Agent gRPC server identity: %w", err)
	}
	clientRoots := x509.NewCertPool()
	if !clientRoots.AppendCertsFromPEM(caCertificatePEM) {
		return nil, nil, nil, fmt.Errorf("Agent CA certificate is invalid")
	}
