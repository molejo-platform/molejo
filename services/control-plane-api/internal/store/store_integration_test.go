...
func createRelease(t *testing.T, storage *Store, workspaceID, actorID int64, project domain.Project, app domain.App, target domain.AppEnvironment) (string, string) {
	...
	if _, err = storage.Pool.Exec(ctx, `INSERT INTO releases(public_id,workspace_id,project_id,app_id,app_environment_id,build_id,commit_sha,image,platform,source_provider,source_repository,source_revision,producer_kind,created_by_principal_id)
		SELECT $1,$2,$3,$4,$5,$6,$7,$8,'linux/amd64','GitHub','molejo/platform',$7,'buildkit',u.principal_id FROM users u WHERE u.id=$9`, releaseID, workspaceID, project.ID, app.ID, target.ID, internalBuildID, commit, image, actorID); err != nil {
		t.Fatal(err)
	}
...
func newIntegrationFixture(t *testing.T) (*Store, int64, int64) {
	...
	if err = storage.Pool.QueryRow(ctx, `INSERT INTO agent_installations(public_id,name,status,cluster_uid,agent_version,kubernetes_version,capabilities_json,created_by,last_seen_at)
		VALUES($1,'Integration cluster','Active','integration-cluster','test','v1.36.3','["runtime.v1alpha1"]'::jsonb,$2,now()) RETURNING id`, clusterPublicID, actorID).Scan(&clusterID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = storage.Pool.Exec(ctx, `INSERT INTO workspace_clusters(workspace_id,installation_id,namespace_name,state,observed_generation)
...
func deploymentAudit(t *testing.T) audit.Event {
	t.Helper()
	return audit.Event{PublicID: newID(t, "aud"), Action: "deployment.create", TargetType: "Deployment", Outcome: audit.Succeeded}
}
