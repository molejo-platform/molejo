...
func TestBuildFailureNeverPromotesAReleaseAndRetryCompletesImmutably(t *testing.T) {
	...
	app, err := storage.CreateApp(ctx, workspaceID, project.PublicID, mustPublicID(t, "app"), "API", "api")
	if err != nil {
		t.Fatal(err)
	}
	...
	build, reused, err := storage.CreateBuild(ctx, workspaceID, actorID, mustPublicID(t, "bld"), project.PublicID, app.PublicID, sha, domain.SHA256([]byte("build-key")), domain.SHA256([]byte("{}")))
	if err != nil || reused || build.CommitSHA != sha || build.Status != domain.BuildPending {
	...
	release, err := storage.CompleteBuild(ctx, claimed, mustPublicID(t, "rel"), image)
	if err != nil || release.BuildPublicID != build.PublicID || release.Image != image || release.CommitSHA != sha {
		t.Fatalf("release=%+v err=%v", release, err)
	}
	if _, err = storage.CompleteBuild(ctx, claimed, mustPublicID(t, "rel"), strings.Replace(image, strings.Repeat("a", 64), strings.Repeat("b", 64), 1)); !errors.Is(err, ErrConflict) {
