...
func TestBuildFailureNeverPromotesAReleaseAndRetryCompletesImmutably(t *testing.T) {
	...
	build, reused, err := storage.CreateBuild(ctx, workspaceID, actorID, mustPublicID(t, "bld"), project.PublicID, app.PublicID, "develop", sha, domain.SHA256([]byte("build-key")), domain.SHA256([]byte("{}")))
	if err != nil || reused || build.SourceBranch != "develop" || build.CommitSHA != sha || build.Status != domain.BuildPending {
