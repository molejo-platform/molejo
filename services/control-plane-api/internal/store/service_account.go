...
func (s *Store) CreateServiceAccount(ctx context.Context, params CreateServiceAccountParams) (automation.ServiceAccount, error) {
	...
	err = tx.QueryRow(ctx, `SELECT p.id,a.id,w.public_id FROM projects p
		...
		WHERE p.workspace_id=$1 AND p.public_id=$2 AND a.public_id=$3
		  AND p.archived_at IS NULL AND a.archived_at IS NULL`, params.WorkspaceID, params.ProjectPublicID, params.AppPublicID).
	...
	if err = tx.QueryRow(ctx, `INSERT INTO principals(public_id,kind,display_name)
		VALUES($1,'ServiceAccount',$2) RETURNING id`, params.ServiceAccountPublicID, params.Name).Scan(&principalID); err != nil {
	...
	if _, err = tx.Exec(ctx, `INSERT INTO service_accounts(principal_id,workspace_id,project_id,app_id,name,created_by_user_id)
		VALUES($1,$2,$3,$4,$5,$6)`, principalID, params.WorkspaceID, projectID, appID, params.Name, params.UserID); err != nil {
		if uniqueConstraint(err) == "service_accounts_app_id_name_key" {
			return automation.ServiceAccount{}, ErrNameConflict
		}
		return automation.ServiceAccount{}, translateDBError(err)
	}
	...
	for _, publicID := range params.DeploymentEnvironmentIDs {
		result, insertErr := tx.Exec(ctx, `INSERT INTO service_account_grants(principal_id,app_id,permission,app_environment_id)
			...
			principalID, appID, automation.PermissionDeploymentCreate, params.WorkspaceID, appID, publicID)
		if insertErr != nil {
	...
	event.TargetPublicID = params.ServiceAccountPublicID
	...
	return automation.ServiceAccount{
		PublicID:                 params.ServiceAccountPublicID,
		WorkspacePublicID:        workspacePublicID,
		ProjectPublicID:          params.ProjectPublicID,
		AppPublicID:              params.AppPublicID,
		Name:                     params.Name,
		Status:                   "Active",
		DeploymentEnvironmentIDs: params.DeploymentEnvironmentIDs,
		CreatedAt:                createdAt,
		UpdatedAt:                updatedAt,
	}, nil
}
