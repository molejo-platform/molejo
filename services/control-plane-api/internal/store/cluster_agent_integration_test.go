...
func TestAgentActivationRejectsUnknownRevokedAndExpiredIdentities(t *testing.T) {
	...
	if _, err := storage.ActivateAgent(t.Context(), newID(t, "agi"), fingerprint, "cluster-test-uid", "test", "v1.36.3", []string{"runtime.v1alpha2"}, workspacecontract.ProvisioningNamespaced, testTrustBundleID, "ags-unknown", now, audit.Event{}); !errors.Is(err, ErrAgentIdentityMismatch) {
	...
	if _, err := storage.ActivateAgent(t.Context(), revokedID, revokedFingerprint, "cluster-test-uid", "test", "v1.36.3", []string{"runtime.v1alpha2"}, workspacecontract.ProvisioningNamespaced, testTrustBundleID, "ags-revoked", now, audit.Event{}); !errors.Is(err, ErrAgentIdentityMismatch) {
	...
	if _, err := storage.ActivateAgent(t.Context(), expiredID, expiredFingerprint, "cluster-test-uid", "test", "v1.36.3", []string{"runtime.v1alpha2"}, workspacecontract.ProvisioningNamespaced, testTrustBundleID, "ags-expired", now, audit.Event{}); !errors.Is(err, ErrAgentIdentityMismatch) {
...
func TestAgentCredentialRenewalIsIdempotentAndRevocable(t *testing.T) {
	...
	if _, err = storage.ActivateAgent(t.Context(), installation.PublicID, oldFingerprint, "cluster-uid", "v0.1.0", "v1.36.3", []string{"runtime.v1alpha2"}, workspacecontract.ProvisioningNamespaced, testTrustBundleID, "ags-superseded", now.Add(time.Minute), audit.Event{}); !errors.Is(err, ErrAgentIdentityMismatch) {
	...
	if _, err = storage.ActivateAgent(t.Context(), installation.PublicID, newFingerprint, "cluster-uid", "v0.1.0", "v1.36.3", []string{"runtime.v1alpha2"}, workspacecontract.ProvisioningNamespaced, testTrustBundleID, replacementSessionID, now.Add(2*time.Hour), audit.Event{}); err != nil {
