...
func TestGitHubRepositoryCanBeSharedByAppsButInstallationCannotBeDisconnectedWhileInUse(t *testing.T) {
	...
	first, err := storage.CreateApp(ctx, workspaceID, project.PublicID, newID(t, "app"), "API", "api")
	if err != nil {
		t.Fatal(err)
	}
	second, err := storage.CreateApp(ctx, workspaceID, project.PublicID, newID(t, "app"), "Worker", "worker")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, app := range []domain.App{first, second} {
		...
		if getErr != nil || source == nil || source.Repository.ID != repository.ID || source.Repository.DefaultBranch != "main" {
