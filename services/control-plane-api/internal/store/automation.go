...
func (s *Store) CreateServiceAccount(ctx context.Context, workspaceID, userID int64, projectPublicID, appPublicID, serviceAccountPublicID, name string, deploymentEnvironmentIDs []string, event audit.Event) (automation.ServiceAccount, error) {
...
func (s *Store) CreateServiceAccountToken(ctx context.Context, workspaceID, userID int64, projectPublicID, appPublicID, serviceAccountPublicID, tokenPublicID string, tokenHash []byte, expiresAt time.Time, event audit.Event) (automation.Token, error) {
	tx, err := s.Pool.Begin(ctx)
	if err != nil {
	...
	err = tx.QueryRow(ctx, `SELECT sa.principal_id FROM service_accounts sa
		...
		WHERE sa.workspace_id=$1 AND pr.public_id=$2 AND a.public_id=$3 AND p.public_id=$4
		  AND p.status='Active'`, workspaceID, projectPublicID, appPublicID, serviceAccountPublicID).Scan(&principalID)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	err = tx.QueryRow(ctx, `INSERT INTO service_account_tokens(public_id,principal_id,token_hash,expires_at)
		VALUES($1,$2,$3,$4) RETURNING public_id,expires_at,revoked_at,last_used_at,created_at`, tokenPublicID, principalID, tokenHash, expiresAt).
