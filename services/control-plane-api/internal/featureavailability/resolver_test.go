...
func TestResolveRuntimePrecedence(t *testing.T) {
	...
	tests := []struct {
		...
		{"old protocol", Facts{ClusterAttached: true, AgentConnected: true, ProtocolCapabilities: []string{"runtime.v1alpha3"}}, Unsupported, ReasonClusterCapabilityIncompatible},
		{"available", Facts{ClusterAttached: true, AgentConnected: true, ProtocolCapabilities: []string{"runtime.v1alpha4"}}, Available, ""},
	}
...
func TestResolveWorkspaceProvisioningKeepsConsentSeparateFromProtocolSupport(t *testing.T) {
	now := time.Now().UTC()
	base := Facts{ClusterAttached: true, AgentConnected: true, ProtocolCapabilities: []string{"runtime.v1alpha4", "workspace-provisioning.v1alpha1"}}
...
func TestUnavailableHistoricalMetricsDoNotAffectApplicationLoop(t *testing.T) {
	...
	features := Resolve(now, Target{}, Facts{
		...
		AgentConnected:       true,
		ProtocolCapabilities: []string{"runtime.v1alpha4"},
