...
func TestWorkspaceProvisioningRequiresCapabilityAndExplicitClusterConsent(t *testing.T) {
	...
	if _, err := storage.Pool.Exec(ctx, `UPDATE agent_installations SET workspace_provisioning_mode='Namespaced',capabilities_json='["runtime.v1alpha4"]'::jsonb WHERE public_id=$1`, testAgentInstallationID); err != nil {
		t.Fatal(err)
	}
