...
func (h *generatedHandler) ListWorkspaceDeployments(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, _ generated.ListWorkspaceDeploymentsParams) {
	_, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), false)
	if ok {
...
func (h *generatedHandler) CreateWorkspaceDeployment(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, _ generated.CreateWorkspaceDeploymentParams) {
	actor, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), true)
	if ok {
		h.server.createDeploymentForWorkspace(w, r, workspace, actor.ID, true)
	}
}

func (h *generatedHandler) GetWorkspaceDeployment(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, deploymentID string) {
	_, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), false)
	if !ok {
		return
	}
...
func (h *generatedHandler) UpdateWorkspaceDeployment(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, deploymentID string, _ generated.UpdateWorkspaceDeploymentParams) {
	actor, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), true)
	if !ok {
		return
	}
	deployment, ok := h.deployment(w, r, workspace.ID, deploymentID)
	if ok {
		h.server.updateDeploymentForWorkspace(w, r, workspace, actor.ID, deployment, true)
	}
}

func (h *generatedHandler) DeleteWorkspaceDeployment(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, deploymentID string, _ generated.DeleteWorkspaceDeploymentParams) {
	actor, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), true)
	if !ok {
		return
	}
	deployment, ok := h.deployment(w, r, workspace.ID, deploymentID)
	if ok {
		h.server.deleteDeployment(w, r, workspace, actor.ID, deployment)
	}
}

func (h *generatedHandler) ListWorkspaceDeploymentOperations(w http.ResponseWriter, r *http.Request, workspaceID generated.WorkspaceId, deploymentID string) {
	_, workspace, ok := h.authorizeWorkspace(w, r, string(workspaceID), false)
	if !ok {
		return
	}
