...
func TestApplyReportsPendingPlanBeforeRunningBackfillAndContract(t *testing.T) {
	...
	scopedDSN, cleanup, err := testsupport.IsolatedPostgres(ctx, dsn, "hierarchy_apply")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func TestApplyIsIdempotentAfterPostHierarchyMigrations(t *testing.T) {
	dsn := strings.TrimSpace(os.Getenv("FRUTO_TEST_DATABASE_URL"))
	if dsn == "" {
		t.Skip("set FRUTO_TEST_DATABASE_URL to run PostgreSQL integration tests")
	}
	...
	ctx := context.Background()
	scopedDSN, cleanup, err := testsupport.IsolatedPostgres(ctx, dsn, "hierarchy_reapply")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	if output, commandErr := command.CombinedOutput(); commandErr != nil {
		t.Fatalf("apply should accept an already migrated schema: %v\n%s", commandErr, output)
	}
}
