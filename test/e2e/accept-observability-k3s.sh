...
for target in statefulset/clickhouse statefulset/victoria-metrics deployment/otel-gateway deployment/otel-cluster daemonset/otel-agent; do
  kubectl --context "$context" -n molejo-observability rollout status "$target" --timeout=60s >/dev/null
done

for component in otel-gateway otel-cluster otel-agent; do
